...
export default function About({ isMenuOpen }: { isMenuOpen: boolean }) {
  ...
  return (
    <section
      id="about"
      ref={ref}
      className="pt-0 sm:pt-6 md:pt-10 lg:pt-20 snap-start max-w-full sm:max-w-350 overflow-hidden mx-auto px-3 sm:px-4 md:px-6 flex items-start sm:items-center sm:justify-center h-fit md:h-screen pb-2 sm:pb-0"
    >
      <div
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-4 md:gap-8 lg:gap-16 items-start mt-20 sm:mt-0">
            ...
            >
              ...
              <h2 className="font-light tracking-tight leading-tight text-foreground text-xl sm:text-2xl md:text-3xl lg:text-5xl mt-1 sm:mt-2">
                <AnimatePresence mode="wait">
                  <motion.p
                    key={lang}
                    initial={{ opacity: 0 }}
            ...
            >
              <AnimatePresence mode="wait">
                ...
                >
                  <span className="block text-foreground/70 font-light leading-relaxed text-[13px] sm:text-sm md:text-[18px]">
                    {lang === "en"
              ...
              >
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className="space-y-1 sm:space-y-2 min-h-fit"
                  >
                    <span className="text-foreground/70 font-mono text-[13px] sm:text-[16px]">
                      {value.number}
                    </span>
                    <h1 className="mt-1 sm:mt-2 font-light text-foreground text-[15px] sm:text-base md:text-lg">
                      {value.title}
                    </h1>
                    <span className="mt-1 sm:mt-2 text-foreground/70 font-light leading-relaxed text-[13px] sm:text-sm md:text-[18px]">
                      {value.description}
                    </span>
                  </motion.div>
                </AnimatePresence>
